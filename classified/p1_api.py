...
async def generate_pdf(questionData: QuestionsList):
    ...
    if level == "A2" or level == "AS":
        ...
        level2 = "IGCSE"
    imagelocation=f"{basePath}/images/unsorted/{level2}/{subject}/p1/"
    ...
    for currentChapterNum in range(1, numOfChapters + 1):
        ...
        for questionNum in range(numOfQuestionsInChapter): # iterates over all the chapters that are in the json in ascending order
    ...
    answerPdf.output(f"{outputdirectory}/1_answers.pdf")
    classifiedPdf.output(f"{outputdirectory}/1.pdf")

    files = [f"{outputdirectory}/1_answers.pdf", f"{outputdirectory}/1.pdf"]
