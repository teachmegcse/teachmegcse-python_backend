...
def make_question_ms(ms_pdf: str, subject_name: str, subject_code: int) -> None:
    ...
    for i in range(3, len(total_pages)):
        ...
        print(pytesseract.image_to_string(this_page)[0])
    ...
    for page in ms_pages:
        ...
        if question_number != int(first_num):
            ...
            if first_json:
                ms_data = [{"fileName": f"{subject_name}_ms_{current_file_number}", "questionNumber": question_number, "paperCode": ms_pdf}]
                first_json = False
            else:
                ms_data.append({"fileName": f"{subject_name}_ms_{current_file_number}", "questionNumber": question_number, "paperCode": ms_pdf})
        ...
        if end_of_question:
            ...
            print(len(current_question), "len")
            output_file = f"{output_folder}/{subject_name}_ms_{current_file_number}.jpg"
            combine_array_images(current_question, output_file)
...
make_question_ms("9702_w19_ms_41.pdf", 'physics', 9702)